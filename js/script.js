...
(function (global) {
  ...
  dc.loadAwards = function () {
    ...
    $ajaxUtils.sendGetRequest(
      ...
      function (awardHtml) {
        ...
        var awardsViewHtml = buildAwardsViewHtml(awardHtml);
      ...
      function (moviesTitleHtml) {
        ...
        $ajaxUtils.sendGetRequest(
          ...
          function (movieHtml) {
            ...
            var moviesViewHtml = buildMoviesViewHtml(
              categoryMovies,
              moviesTitleHtml,
              movieHtml
            );
  ...
  function buildMoviesViewHtml(categoryMovies, moviesTitleHtml, movieHtml) {
    moviesTitleHtml = insertProperty(
      moviesTitleHtml,
      "name",
    ...
    for (var i = 0; i < movies.length; i++) {
      ...
      var html = movieHtml;
      html = insertProperty(html, "short_name", movies[i].short_name);
      html = insertProperty(html, "catShortName", catShortName);
      html = insertMoviePrice(html, "price_small", movies[i].price_small);
      html = insertMoviePortionName(
        html,
        "small_portion_name",
        movies[i].small_portion_name
      );
      html = insertMoviePrice(html, "price_large", movies[i].price_large);
      html = insertMoviePortionName(
        html,
        "large_portion_name",
        movies[i].large_portion_name
      );
      html = insertProperty(html, "name", movies[i].name);
      html = insertProperty(html, "description", movies[i].description);
  ...
  function insertMoviePrice(html, pricePropName, priceValue) {
  ...
  function insertMoviePortionName(html, portionPropName, portionValue) {
